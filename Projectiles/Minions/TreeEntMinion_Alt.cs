...
{
	public class TreeEntMinion_Alt : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (minionCheck)
			{
				...
				if (projectile.localAI[0] >= (float)Main.rand.Next(900, 1200))
				{
					...
					{
						int num684 = Dust.NewDust(projectile.Center + Vector2.UnitX * -(float)projectile.direction * 8f - Vector2.One * 5f + Vector2.UnitY * 8f, 3, 6, DustType<IndendirtDust>(), -(float)projectile.direction, 1f, 0, default(Color), 1f);
